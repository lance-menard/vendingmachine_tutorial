...
void OrderBeverageServiceHandler::PlaceOrder(std::string& _return, const int64_t city){
    ...
    // 3. Call the beverage preference service
    auto beverage_reference_client_wrapper = _beverage_preference_client_pool->Pop();
    if (!beverage_reference_client_wrapper) {
      ServiceException se;
      se.errorCode = ErrorCode::SE_THRIFT_CONN_ERROR;
      se.message = "Failed to connect to beverage-preference-service";
      throw se;
    }
    auto beverage_client = beverage_reference_client_wrapper->GetClient();

    try {
      beverage_client->GetBeverage(_return, weatherType == WeatherType::type::WARM ? BeverageType::type::COLD : BeverageType::type::HOT);
    } catch (...) {
      _beverage_preference_client_pool->Push(beverage_reference_client_wrapper);
